...
if __name__ == "__main__":
	...
	np.save(f"results/heuristic_{args.env}_{args.time_steps}_{order}_{args.alpha}_{args.seed}.npy", np.array(results.stored_rewards))

	model.save(f"results/heuristic_{args.env}_{args.time_steps}_{order}_{args.alpha}_{args.seed}")
	if not os.path.exists(f"results/heuristic_{args.env}_{args.time_steps}_{order}_{args.alpha}_{args.seed}"):
		os.mkdir(f"results/heuristic_{args.env}_{args.time_steps}_{order}_{args.alpha}_{args.seed}")
	stats_path = os.path.join(f"results/heuristic_{args.env}_{args.time_steps}_{order}_{args.alpha}_{args.seed}.pkl")#, "vec_normalize.pkl")
	env.save(stats_path)

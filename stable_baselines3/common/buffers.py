...
class HeronBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.factors = np.zeros((self.buffer_size, self.n_envs, self.factor_dim), dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(
        ...
        if factor is not None:
            self.factors[self.pos] = np.array(factor).copy()
        self.rewards[self.pos] = np.array(reward).copy()
